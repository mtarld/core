...
final class PropertySchemaLessThanRestrictionTest extends TestCase
{
    ...
    public static function supportsProvider(): \Generator
    {
        ...
        if (!method_exists(ApiProperty::class, 'getPhpType')) {
            ...
            yield 'supported int/float with union types' => [new LessThan(['value' => 10]), (new ApiProperty())->withPhpType(Type::union(Type::int(), Type::float())), true];
            yield 'supported int' => [new LessThan(['value' => 10]), (new ApiProperty())->withPhpType(Type::int()), true];
            yield 'supported float' => [new LessThan(['value' => 10.99]), (new ApiProperty())->withPhpType(Type::float()), true];
            yield 'supported negative' => [new Negative(), (new ApiProperty())->withPhpType(Type::int()), true];
            yield 'not supported negative or zero' => [new NegativeOrZero(), (new ApiProperty())->withPhpType(Type::int()), false];
            yield 'not supported property path' => [new LessThan(['propertyPath' => 'greaterThanMe']), (new ApiProperty())->withPhpType(Type::int()), false];
        }
    }
    ...
    public function testCreate(): void
    {
        ...
        if (!method_exists(ApiProperty::class, 'getPhpType')) {
            ...
            self::assertEquals([
                'exclusiveMaximum' => 10,
            ], $this->propertySchemaLessThanRestriction->create(new LessThan(['value' => 10]), (new ApiProperty())->withPhpType(Type::int())));
        }
    }
}
