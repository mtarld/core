...
final class PropertySchemaGreaterThanRestrictionTest extends TestCase
{
    ...
    public static function supportsProvider(): \Generator
    {
        ...
        if (!method_exists(ApiProperty::class, 'getPhpType')) {
            ...
            yield 'supported int/float with union types' => [new GreaterThan(['value' => 10]), (new ApiProperty())->withPhpType(Type::union(Type::int(), Type::float())), true];
            yield 'supported int' => [new GreaterThan(['value' => 10]), (new ApiProperty())->withPhpType(Type::int()), true];
            yield 'supported float' => [new GreaterThan(['value' => 10.99]), (new ApiProperty())->withPhpType(Type::float()), true];
            yield 'supported positive' => [new Positive(), (new ApiProperty())->withPhpType(Type::int()), true];
            yield 'not supported positive or zero' => [new PositiveOrZero(), (new ApiProperty())->withPhpType(Type::int()), false];
            yield 'not supported property path' => [new GreaterThan(['propertyPath' => 'greaterThanMe']), (new ApiProperty())->withPhpType(Type::int()), false];
        }
    }
    ...
    public function testCreate(): void
    {
        ...
        if (!method_exists(ApiProperty::class, 'getPhpType')) {
            ...
            self::assertEquals([
                'exclusiveMinimum' => 10,
            ], $this->propertySchemaGreaterThanRestriction->create(new GreaterThan(['value' => 10]), (new ApiProperty())->withPhpType(Type::int())));
        }
    }
}
