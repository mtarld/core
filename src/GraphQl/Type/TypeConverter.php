...
final class TypeConverter implements TypeConverterInterface
{
    ...
    public function convertPhpType(Type $type, bool $input, Operation $rootOperation, string $resourceClass, string $rootResource, ?string $property, int $depth): GraphQLType|string|null
    {
        ...
        if ($type->isA(TypeIdentifier::STRING) || $type->isA(\DateTimeInterface::class)) {
            return GraphQLType::string();
        }

        if ($type->isA(TypeIdentifier::ARRAY) || $type->isA(TypeIdentifier::ITERABLE)) {
    ...
    /**
     * @deprecated since 3.3, use "convertPhpType" instead
     *
     * {@inheritdoc}
     */
    ...
    {
        trigger_deprecation('api-platform/graphql', '3.3', 'The "%s()" method is deprecated, use "%s::convertPhpType()" instead.', __METHOD__, self::class);
