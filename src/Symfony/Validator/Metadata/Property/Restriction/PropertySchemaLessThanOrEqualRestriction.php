...
final class PropertySchemaLessThanOrEqualRestriction implements PropertySchemaRestrictionMetadataInterface
{
    ...
    public function supports(Constraint $constraint, ApiProperty $propertyMetadata): bool
    {
        ...
        if (method_exists($propertyMetadata, 'getPhpType')) {
            if (!$constraint instanceof LessThanOrEqual || !is_numeric($constraint->value)) {
                return false;
            }
        ...
        return $constraint instanceof LessThanOrEqual && is_numeric($constraint->value) && \count($types) && array_intersect($types, [LegacyType::BUILTIN_TYPE_INT, LegacyType::BUILTIN_TYPE_FLOAT]);
    }
}
