...
abstract class AbstractItemNormalizer extends AbstractObjectNormalizer
{
    ...
    {
        trigger_deprecation('api-platform/serializer', '4.1', 'TODO mtarld', __METHOD__, self::class);
    ...
    protected function validateAttributeType(string $attribute, Type $type, mixed $value, ?string $format = null, array $context = []): void
    {
        if ($type->isIdentifiedBy(TypeIdentifier::FLOAT) && null !== $format && str_contains($format, 'json')) {
            $isValid = Type::union(Type::int(), Type::float())->accepts($value);
        } else {
    ...
    protected function denormalizeObjectCollection(string $attribute, ApiProperty $propertyMetadata, Type $type, string $className, mixed $value, ?string $format, array $context): array
    {
        ...
        foreach ($value as $index => $obj) {
            ...
            if ($type instanceof CollectionType && !$type->getCollectionKeyType()->accepts($index)) {
                throw NotNormalizableValueException::createForUnexpectedDataType(\sprintf('The type of the key "%s" must be "%s", "%s" given.', $index, $type->getCollectionKeyType(), \gettype($index)), $index, [(string) $type->getCollectionKeyType()], ($context['deserialization_path'] ?? false) ? \sprintf('key(%s)', $context['deserialization_path']) : null, true);
            }
    ...
    protected function getAttributeValue(object $object, string $attribute, ?string $format = null, array $context = []): mixed
    {
        ...
        if (!method_exists(ApiProperty::class, 'getPhpType')) {
            ...
            // TODO mtarld check every foreach composite to see if null is an issue
        ...
        foreach ($types as $type) {
            if ($type instanceof CollectionType && $type->getCollectionValueType()->isSatisfiedBy($typeIsResourceClass)
                || !$type instanceof Type && $type->isCollection() && ($collectionValueType = $type->getCollectionValueTypes()[0] ?? null) && ($className = $collectionValueType->getClassName()) && $this->resourceClassResolver->isResourceClass($className)
            ) {
                $childContext = $this->createChildContext($this->createOperationContext($context, $className), $attribute, $format);
            ...
            if ($type instanceof Type && $type->isSatisfiedBy($typeIsResourceClass)
                || !$type instanceof Type && ($className = $type->getClassName()) && $this->resourceClassResolver->isResourceClass($className)
            ) {
                $childContext = $this->createChildContext($this->createOperationContext($context, $className), $attribute, $format);
            ...
            if ($type instanceof CollectionType || !$type instanceof Type && 'array' === $type->getBuiltinType()) {
                if ($type instanceof CollectionType && ($subType = $type->getCollectionValueType()) instanceof ObjectType) {
                    $context = $this->createOperationContext($context, $subType->getClassName());
                } elseif (!$type instanceof Type && $className = ($type->getCollectionValueTypes()[0] ?? null)?->getClassName()) {
    ...
    private function createAndValidateAttributeValue(string $attribute, mixed $value, ?string $format = null, array $context = []): mixed
    {
        ...
        foreach ($types as $t) {
            $isNullable = !$t instanceof Type ? $t->isNullable() : $type->isNullable();
            ...
            if ($collectionValueType instanceof Type && $collectionValueType->isSatisfiedBy($typeIsResourceClass)
                || !$t instanceof Type && $t->isCollection() && null !== $collectionValueType && null !== ($className = $collectionValueType->getClassName()) && $this->resourceClassResolver->isResourceClass($className)
            ) {
            ...
            if ($t instanceof Type && $t->isSatisfiedBy($typeIsResourceClass)
                || !$t instanceof Type && null !== ($className = $t->getClassName()) && $this->resourceClassResolver->isResourceClass($className)
            ) {
                $resourceClass = $this->resourceClassResolver->getResourceClass(null, $className);
            ...
            if (is_array($value) && (
                $t instanceof CollectionType && $t->getCollectionValueType()->isSatisfiedBy($typeIsResourceClass)
                || !$t instanceof Type && $t->isCollection() && null !== $collectionValueType && null !== ($className = $collectionValueType->getClassName()))
            ) {
                if (!$this->serializer instanceof DenormalizerInterface) {
                    throw new LogicException(\sprintf('The injected serializer must be an instance of "%s".', DenormalizerInterface::class));
                }
            ...
            if ($t instanceof ObjectType
                || !$t instanceof Type && null !== $t->getClassName()
            ) {
                if (!$this->serializer instanceof DenormalizerInterface) {
                    throw new LogicException(\sprintf('The injected serializer must be an instance of "%s".', DenormalizerInterface::class));
                }
            ...
            if (\is_string($value) && (XmlEncoder::FORMAT === $format || CsvEncoder::FORMAT === $format)) {
                if ('' === $value && $isNullable && (
                    ...
                    || !$t instanceof Type && \in_array($t->getBuiltinType(), [LegacyType::BUILTIN_TYPE_BOOL, LegacyType::BUILTIN_TYPE_INT, LegacyType::BUILTIN_TYPE_FLOAT], true)
                )) {
                    return null;
                }

                $typeIdentifier = $t instanceof Type ? $t->getTypeIdentifier() : TypeIdentifier::tryFrom($t->getBuiltinType());
